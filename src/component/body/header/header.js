...
const [fade , setFade] = useState(false)
  ...
  return (
    <section className="z-30 relative mt-24 flex flex-row py-[50px]">
      <img
        src={info.addres}
        className={`anim w-full h-full z-10 absolute bottom-0 top-0 ${ fade ? 'hiden' : ' '}`}
        alt=""
      />
      ...
      <div className="p-16 z-30 w-1/3">
        <div className="space-y-4">
          <h1 className={`text-3xl anim font-bold text-white ${ fade ? 'hiden' : ' '}`}>{info.name}</h1>
          <p className={`text-lg text-justify h-[200px] anim text-white ${ fade ? 'hiden' : ' '} `}>
            {info.title}
          </p>
        </div>
        <div className="flex flex-row my-5 items-center justify-center">
          <div>
            ...
            <span className="text-white">
              <span className={`${ fade ? 'hiden' : ' '} text-yellow-500 anim text-xl font-bold`}>{info.rating}  </span>/100</span>
          </div>
          <div className="ml-10">
            <img src="img/icon/uiw_date.svg" alt="" className="inline mr-2" />
            <span className={`text-white anim ${ fade ? 'hiden' : ' '}`}>{info.date}</span>
          </div>
        </div>
